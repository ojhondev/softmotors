...
export function vehicleBadge(
  v: VehicleCardData,
): { label: string; tone: "dark" | "green" } | null {
...
export function VehicleCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-[var(--radius-card)] border border-line bg-surface transition-shadow hover:shadow-card">
      <FavoriteButton
        slug={v.slug}
      ...
      <Link href={`/veiculo/${v.slug}`} className="block">
        ...
        <div className="p-4">
          ...
          <div className="mt-1 text-[12.5px] text-ink-muted">{cityLabel ?? `${v.color}`}</div>
          <div className="mt-3 font-display text-xl font-extrabold tracking-tight">
            {money(v.price)}
          </div>
          {parcela && (
            <div className="text-[12px] text-ink-muted">ou 48x de {moneyCents(parcela)}</div>
          )}
        </div>
      </Link>
    </div>
